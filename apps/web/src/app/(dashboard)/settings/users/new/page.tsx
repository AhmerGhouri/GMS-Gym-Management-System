...
export default function AddUserPage() {
  ...
  const customRoles = Array.isArray(rolesData) ? rolesData : rolesData?.data || [];
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post('/users', {
        ...
        role: customRoleId ? 'RECEPTIONIST' : role, customRoleId,
      });
      toast({ title: 'User Created', description: 'The new user can now sign in.', variant: 'success' });
      router.push('/settings');
    } catch (error: any) {
      toast({ title: 'Error', description: error.response?.data?.message || 'Unable to create user.', variant: 'destructive' });
    } finally { setSaving(false); }
  };

  return <div className="max-w-xl space-y-6"><div><h1 className="text-3xl font-bold text-white">Add User</h1><p className="text-slate-400">Create a staff account and assign its role.</p></div><Card className="border-slate-800 bg-slate-900/50"><CardHeader><CardTitle className="text-white">User details</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="space-y-4"><div className="grid grid-cols-2 gap-4"><div><Label>First name</Label><Input name="firstName" required /></div><div><Label>Last name</Label><Input name="lastName" required /></div></div><div><Label>Email</Label><Input name="email" type="email" required /></div><div><Label>Password</Label><Input name="password" type="password" minLength={8} required /></div><div><Label>Role</Label><Select value={role} onValueChange={setRole}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="GYM_MANAGER">Gym Manager</SelectItem><SelectItem value="RECEPTIONIST">Receptionist</SelectItem><SelectItem value="SUPER_ADMIN">Super Admin</SelectItem>{customRoles.map((customRole: any) => <SelectItem key={customRole.id} value={`custom:${customRole.id}`}>{customRole.name}</SelectItem>)}</SelectContent></Select></div><Button disabled={saving} className="bg-cyan-600">{saving ? 'Creating…' : 'Create User'}</Button></form></CardContent></Card></div>;
}
