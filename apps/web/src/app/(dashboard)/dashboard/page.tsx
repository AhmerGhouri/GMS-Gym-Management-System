  ...
  const statCards = [
    {
      ...
      value: isPrivacyMode ? '***' : (stats.monthlyNewMembers ?? 0).toString(),
      icon: UserPlus,
      change: change(stats.changes?.monthlyNewMembers),
      trend: (stats.changes?.monthlyNewMembers || 0) >= 0 ? ('up' as const) : ('down' as const),
    ...
    },
    {
      ...
      value: isPrivacyMode ? '***' : (stats.todayAttendance ?? 0).toString(),
      icon: CalendarDays,
      change: change(stats.changes?.todayAttendance),
      trend: (stats.changes?.todayAttendance || 0) >= 0 ? ('up' as const) : ('down' as const),
    ...
    ...(hasReportsAccess ? [{
      ...
      trend: (stats.changes?.monthlyRevenue || 0) >= 0 ? ('up' as const) : ('down' as const),
    ...
    }] : []),
    {
      ...
      trend: (stats.changes?.monthlyExpiredMembers || 0) > 0 ? ('down' as const) : ('up' as const),
    ...
    },
    {
      ...
      value: isPrivacyMode ? '***' : (stats.activeMembers ?? 0).toString(),
      icon: UserCheck,
      change: '',
      trend: 'up' as const,
      color: 'text-emerald-600 dark:text-emerald-500',
